...
const Product360Viewer = ({ images, productTitle }) => {
  ...
  const [autoRotate, setAutoRotate] = useState(true); // Auto slideshow enabled by default
  ...
  useEffect(() => {
    if (autoRotate && images && images.length > 0) {
      ...
      }, 2000); // Changed to 2 seconds for better viewing
    } else {
  ...
  const handleMouseMove = (e) => {
    if (isDragging) {
      ...
      // Calculate mouse position relative to image for zoom
    ...
    <div className="product-360-viewer">
      ...
      <div
        ...
        onMouseLeave={() => {
          handleMouseUp();
          setIsZooming(false);
          // Resume auto-rotate if it was running before zoom
        ...
        onMouseEnter={() => {
          setIsZooming(true);
          // Pause auto-rotate when hovering for zoom
          if (autoRotate) {
      ...
      >
        <img
          ...
          style={{
            ...
            transform: isZooming && !isDragging ? 'scale(2)' : 'scale(1)',
