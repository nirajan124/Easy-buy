...
router.post('/login', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error(`Password comparison error for email: ${normalizedEmail}`, error);
      return res.status(401).json({ success: false, message: 'Invalid email or password' });
    }
    ...
    // But allow login if role doesn't match - just use the actual user role
    // Only block if explicitly trying to login as admin but user is not admin
